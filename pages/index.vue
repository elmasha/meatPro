              ...
              >
                <div class="d-flex align-center justify-space-between mb-3 mb-sm-4">
                  <span 
                    ...
                    :class="parseFloat(plan.price_kes) > 0 ? 'red--text text--lighten-3' : 'grey--text'"
                  >{{ (plan.name || 'PLAN').toUpperCase() }}</span>
                  <v-chip v-if="parseFloat(plan.price_kes) > 0" color="white" x-small label class="red--text font-weight-bold"
                    >Best Value</v-chip
                  >
                </div>
                <div class="d-flex align-baseline mb-4 mb-sm-6">
                  <span class="text-h3 text-sm-h2 font-weight-black" :class="parseFloat(plan.price_kes) === 0 ? 'grey--text text--darken-3' : ''">
                    {{ parseFloat(plan.price_kes) === 0 ? 'Free' : 'KES ' + formatNumber(plan.price_kes) }}
                  </span>
                  <span v-if="parseFloat(plan.price_kes) > 0" class="text-body-2 text-sm-subtitle-1 ml-2" :class="parseFloat(plan.price_kes) > 0 ? 'red--text text--lighten-3' : ''"
                    >/mo</span
                  >
                </div>
                <v-divider class="mb-4 mb-sm-6" :class="parseFloat(plan.price_kes) > 0 ? 'red darken-2' : ''" />
                <v-list dense :dark="parseFloat(plan.price_kes) > 0" class="mb-6 mb-sm-8 transparent pa-0">
                  <v-list-item
                    v-for="feature in parseFeatures(plan.features)"
                    :key="feature"
                    class="px-0 mb-1 mb-sm-2"
                  >
                    <v-icon :color="parseFloat(plan.price_kes) > 0 ? 'green lighten-2' : 'green'" small class="mr-3">mdi-check-circle</v-icon>
                    <span class="text-body-2" :class="parseFloat(plan.price_kes) > 0 ? 'red--text text--lighten-4' : 'grey--text text--darken-1'">{{ feature }}</span>
                  </v-list-item>
                </v-list>
                <v-btn
                  block
                  :outlined="parseFloat(plan.price_kes) === 0"
                  :color="parseFloat(plan.price_kes) === 0 ? 'red darken-2' : 'white'"
                  depressed
                  :large="!$vuetify.breakpoint.xs"
                  :x-large="!$vuetify.breakpoint.xs && !$vuetify.breakpoint.sm"
